...
- **Workflow notes**: workflow definitions stored as notes under `{notor_dir}/workflows/`, providing structured step-by-step instruction sets that guide how the AI approaches specific tasks (not prompt templates). Frontmatter properties drive behavior (triggers, scheduling, persona assignment via `notor-workflow-persona`). At execution time, resolved workflow content is wrapped in `<workflow_instructions>` XML tags — modeled after Cline's `<explicit_instructions>` pattern — signaling to the AI that the content is authoritative guidance to follow methodically.
- **`<include_note>` tag**: dynamically inject note contents (or note sections) into context at execution time using a self-closing XML-style tag (`<include_note path="..." />`). The `path` attribute accepts either a vault-relative file path or an Obsidian wikilink (`[[Note Title]]`); wikilink syntax is recommended because Obsidian's built-in link-updater automatically rewrites it when the referenced note is renamed or moved. Supported in workflow notes (inline and attached modes), system prompts (global and persona), and vault-level rule files (inline mode only).
