...
- Cron expression parsing and scheduling is handled by `croner` v10.x — a lightweight JavaScript library (27.3 KB minified / 8.1 KB gzipped, zero dependencies, ESM-compatible, fully browser/Electron-safe) bundled into the plugin. No external cron daemon or OS-level scheduling is required. `croner` provides both cron parsing and in-process job scheduling with dynamic start/stop, validation via `CronPattern` constructor, per-job timezone support via `Intl.DateTimeFormat`, and shorthand aliases (`@daily`, `@weekly`, etc.).
