...
Three new layers sit between `WebSearchTool.execute()` and the HTTP transport:

```
WebSearchTool.execute()
  │
  │  validate input, call queue
  │
  ▼
WebSearchQueue  (plugin-level singleton)
  │
  │  select provider (priority / round-robin)
...
`WebSearchTool` remains a thin coordinator: validate input → call queue → filter results (domain denylist) → format output. All provider selection, rate limiting, and fallback logic lives in the queue.
...
The `WebSearchQueue` is a singleton created once in `main.ts` and shared by all `WebSearchTool` instances (including those invoked from sub-agent dispatch loops). It accepts search requests, selects providers, enforces per-provider delays, and handles fallback.
...
export class WebSearchQueue {
   ...
   *
   * This is the ONLY public entry point for web searching.
   */
  async search(
    query: string,
    numResults: number,
...
### 5.2 Per-Provider Lanes

The queue maintains one lane per registered provider:

```typescript
interface ProviderLane {
  /** Timestamp (ms) of the last completed request to this provider. */
  lastRequestTime: number;
  /** FIFO queue of pending requests awaiting this provider's delay window. */
...
1. Caller enters the lane via `acquireLane(providerType)`.
2. If `Date.now() - lane.lastRequestTime >= delayMs`, the caller proceeds immediately.
3. Otherwise, the caller is enqueued and a Promise resolves after the remaining delay.
4. After the request completes (success or failure), `lane.lastRequestTime` is updated and the next waiter (if any) is scheduled.
...
**Across lanes, requests are fully concurrent.** A DDG request waiting through its 1500ms delay does not block a Tavily request from firing immediately.
...
2. Queue resolves the ordered fallback chain:
...
3. For the selected provider:
   a. Acquire the provider's lane (wait for delay if needed)
   b. Call provider.search(query, numResults, timeoutMs)
   c. Release the lane (update lastRequestTime, unblock next waiter)
...
src/tools/web-search/
  ...
  queue.ts                   WebSearchQueue singleton
...
| `src/tools/web-search/queue.ts` | **Create** — `WebSearchQueue` |
...
| `src/main.ts` | **Modify** — Add `_searchProviderRegistry` and `_webSearchQueue` singletons; update `WebSearchTool` registration |
...
| `WebSearchQueue` | Delay enforcement: two requests to same provider spaced by at least `delay_ms`. Round-robin: 3 providers, 6 requests, verify distribution. Fallback: first provider fails → second provider used. All fail → aggregated error. No available providers → immediate descriptive error. |
