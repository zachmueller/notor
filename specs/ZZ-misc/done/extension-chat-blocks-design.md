...
- Orchestrator changes from bare `dispatchOnConversationStart(...)` to `await dispatchOnConversationStart(...)`. This requires: (1) changing the function signature from `void` to `async ... Promise<void>`, (2) adding an `emitLoadingBlock?: (kind: string) => Message` callback parameter — the orchestrator wires this to `ConversationManager.addMessage()` at the call site, keeping `hook-events.ts` decoupled from the conversation system, (3) restructuring the internal fire-and-forget IIFE — blocking automations awaited directly in the function body, non-blocking automations remain fire-and-forget, (4) adding per-automation timeout logic. The session snapshot (created after the dispatch in the orchestrator) must be verified to include messages emitted by blocking automations.
...
| `src/chat/conversation.ts` | Add `source_extension` + `exclude_from_compaction` + `transient` fields to `addMessage()` params and body; add `updateMessage()` (in-memory only, no JSONL persistence) + `onMessageUpdated` callback |
| `src/chat/history.ts` | Add `addMessageToConversation(conversationId, params)` for non-active conversation emission |
