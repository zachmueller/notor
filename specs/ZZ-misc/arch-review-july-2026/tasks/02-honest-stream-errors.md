...
- [x] **1.1** `run-loop/types.ts`: extend the union (`RunStopReason`, :238–244) with
...
- [x] **1.2** `run-loop.ts` return sites (verified at c0d21e9): stream-error return around :247 →
...
- [x] **1.3** `sub-agent-runner.ts`: widen `SubAgentStopReason` (:45–49) with
...
- [x] **1.4** `use-subagent.ts:478`: `success: result.stopReason !== "error"` (matches
...
- [x] **2.1** In `resolveEmission` (`step-turn-executor.ts:413–436`), before the `.capped`
      fallthrough:
      ```ts
...
- [x] **2.2** Thread `result.errorMessage` into the call — `step-turn-executor.ts:330` passes
      only `result.stopReason` today; pass the result or add a param.
- [x] **2.3** Add `".stream_error"` to **all three** recognition lists (they must stay in sync):
...
- [x] **2.4** Update the orchestration-creator persona docs (`builtin-personas.ts:423,427`) to
      list the fourth failure channel.
      > **Deviation (stale premise):** lines 423/427 do NOT enumerate the failure channels —
      > 423 is a code-step-throw note (`{step}.code_error`) and 427 a `callTool` caveat; neither
      > lists `.capped`/`.no_emit`/`.code_error` as a set. There was no three→four list to extend.
      > Instead added a new bullet in the persona's "Verification + deterministic routing
      > discipline" section documenting all four runtime failure channels
      > (`.capped`/`.no_emit`/`.code_error`/`.stream_error`) with when-to-subscribe guidance.
- [x] **2.5** No new surfacing code: with no explicit subscriber, the existing fallback
...
- [x] Design notes to preserve in code comments / PR: `.capped` is *not* reused because it means
...
- [x] **3.1 Wind-down turn accounting.** `finishWithWindDown` (`run-loop.ts:395–469`) runs one
...
- [x] **3.2 Errored-turn cost.** The main-loop decrement (:224–230) runs *before* the
...
- [x] **3.3 Child cost visibility** *(needs Task 01 A.4 landed)*: add optional
...
- [x] RunLoop error mapping (`run-loop.test.ts`, `mockProvider` seam at :25–40): stream
      `{type:"error"}` → `stopReason:"error"` + `errorMessage`; abort → `"cancelled"`; budget
      decremented; `onTurnComplete` fired.
- [x] Wind-down accounting (`run-loop.test.ts`): capped run → cell decremented for the wind-down
      call, hook fired for it.
- [x] **Emission matrix** — new `step-turn-executor.emission.test.ts` (or extend
...
- [x] Fallback recognition (`fallback-coordinator.test.ts`, pattern at :53–63): unsubscribed
      `.stream_error` → diagnosable FLOW_ERROR.
- [x] Validator exemption (`flow-parser.test.ts`, pattern at :270–273): a step subscribing to
      `x.stream_error` passes validation.
- [x] Sub-agent surface (`sub-agent-runner.test.ts`): flip :334 (provider stream error →
...
- [x] Budget rebuild with child costs (`session-recovery.test.ts`): log with
...
- [x] `tsc` + suite green.
- [x] Manual: force/stub a Bedrock throttle mid-flow → flow terminates with FLOW_ERROR naming
...
- [x] Grep gate: `grep -rn '"completed"' src/run-loop/run-loop.ts` — no error/cancel path
      returns it.
- [x] Changelog notes: sub-agent parents now get `success: false` on stream errors; flows without
