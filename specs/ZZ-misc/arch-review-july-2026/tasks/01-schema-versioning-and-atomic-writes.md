...
- [ ] Add `schema_version?: number` to `Conversation` (`src/types.ts:24` region).
- [ ] Set it at creation: `createConversationFile` (`src/chat/history.ts:171–177`) and
...
- [ ] `writeSubAgentConversation` builds its metadata literal directly (`history.ts:430–443`) —
      add the field explicitly there.
- [ ] Readers: normalize `header.schema_version ??= 1` in `loadConversation` (:541–547) and
      `listConversations` (:644–660).
- [ ] Verify (don't assume) that `toggleFavorite` (:363) and `updateConversationHeader` preserve
      the field — both spread the parsed header; lock with a test (Phase C).

### A.2 Checkpoint JSON

- [ ] Add required `schema_version: number` to `Checkpoint` (`src/types.ts:389–406`).
- [ ] Stamp at the single construction site (`src/chat/checkpoint.ts:97–106`).
- [ ] Reader `??= 1` normalization in `CheckpointStorage.load` (`storage.ts:92–93`) and
...
- [ ] While touching the blind `JSON.parse(raw) as Checkpoint` cast: add minimal shape validation
...
- [ ] Future-version gate: `schema_version > 1` → skip-with-warn, not crash.

### A.3 Memory note frontmatter

- [ ] Add a `notor-schema: 1` line in both serializers: `serializeNote`
      (`src/memory/note-format.ts:25–31`) and `serializePendingNote` (:89–107).
- [ ] Readers (`parseNote` :40–71, `parsePendingNote` :109–143) extract with `?? 1` default.
- [ ] Confirm `patchFrontmatterField` (:156–184) composes (it inserts after the last `notor-*`
      line — should be fine; assert in a test).
- [ ] PR note: `MemoryNote` is extension-API-visible (`runtime-context/types.ts:283–293`); an
      optional added field is non-breaking.

### A.4 Orchestration session log ← **the one Tasks 02/03 wait on**

- [ ] Version the **stream, not every entry**: add `schema_version: number` to
...
- [ ] `SessionLogReader.parse` (`session-log-reader.ts:52–101`): read it off the first entry when
      present, default 1, expose on `ParsedSessionLog`.
- [ ] Recovery (`session-recovery.ts`) refuses `> 1` with a diagnosable scan error — the loud
      error path at `launch.ts:1151–1155` already exists for this shape of failure.
- [ ] Leave the truncated-final-line tolerance (`session-log-reader.ts:87–97`) untouched.

### A.5 Step-conversation JSONL header

- [ ] One field in `buildStepConversationHeader`
      (`src/orchestration/step-conversation-store.ts:76–104`).
- [ ] Default-on-read in the run-tree scan (`run-tree-view.ts:201–225`) and both backfills
...
- [ ] Field on `OrchestrationSessionMeta` (`src/orchestration/types.ts:150–169`), stamped in
      `createSession` (`session-manager.ts:131–141`).
- [ ] Default in `readMeta` (:153–157 — choke point for two of the three readers) **and** in the
...
- [ ] `appendMessage` (`history.ts:202–203`) and `appendStaleState` (:237–238) →
...
- [ ] **Sharp edge:** `adapter.append` creates-if-absent. The current catch-fallback that writes
...
- [ ] New shared helper `src/utils/atomic-write.ts`:
      `atomicRewrite(adapter, path, mutate: (content: string) => string)` via `adapter.process`
      (documented atomic, `obsidian.d.ts:1649`).
- [ ] Use it at the direct-adapter sites: `toggleFavorite` (`history.ts:370`),
      `updateConversationHeader` (:328), `backfillParentEdge` (`launch.ts:1071`).
- [ ] For the seam-mediated sites — `backfillNextEdge` (via `StepConversationFs.write`,
...
- [ ] Per-session promise chain on `OrchestrationSessionManager` (the `SessionLog.writeChain`
...
- [ ] Batch creates stay plain writes (`createConversationFile`, `importConversation`,
...
- [ ] Round-trip stamp per format: `session-manager.test.ts`, `session-log.test.ts` +
...
- [ ] Legacy tolerance (load-bearing): hand-crafted artifact *without* the field parses,
      defaults to 1, nothing throws — per format.
- [ ] Future-version gate: `schema_version: 2` → recovery scan error / checkpoint
      skip-with-warn.
- [ ] Append conversion: upgrade the `makeVault` fake in `history.test.ts:23–36` (write is a
...
- [ ] Header preservation: `toggleFavorite` / both backfills preserve unknown fields including
      `schema_version`.
- [ ] session.json serialization: two concurrent `updateStatus` calls both land.
...
- [ ] Grep gate: `grep -rn "adapter.write(.*existing" src/chat/history.ts` → zero hits.
