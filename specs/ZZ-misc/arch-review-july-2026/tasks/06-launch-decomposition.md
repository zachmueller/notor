...
- [ ] Cross-edges to plan for, not fight: run-lifecycle's `launchOrchestration` ↔
...
- [ ] `requestOrchestrationInput`: inject as the runner's `requestUserInput` callback from the
...
- [ ] Both finalize sites (launch + resume) must keep calling `maybeWriteFailureReport` and
...
- [ ] Shared-by-reference run state must keep flowing as the same instances across the new
...
- [ ] Suite green on pure moves before proceeding.

## Phase 2 — `OrchestrationHost` + refactors (commit 2)

- [ ] **2.1** New `src/orchestration/host.ts`:
      ```ts
      ...
      ```
- [ ] **2.2** `NotorPlugin implements OrchestrationHost` (everything already exists; the clause
...
- [ ] **2.3** `buildUtils(plugin, …)` is the one hard case — wrap as
...
- [ ] **2.4** Dedupe the dispatcher assembly: the two runtime factories duplicate it
      ...
      post-Task-04 shape.
- [ ] **2.5** Single owner for step-conversation header surgery: move `backfillParentEdge` into
...
- [ ] **2.6** Ledger fs injection (`reconcileChildLedger`/`resolveChildEntryConversationId` take
      a `RecoveryFs`-shaped reader) — **done by Task 03 Phase 3.1**; verify, skip if so.
- [ ] **2.7** Fix or delete the stale `reference-flows.ts:1–15` comment claiming the reference
      ...
      record the decision in the comment.
- [ ] Do **not** fold in F13's `discoverFlows()` caching; keep this PR mechanical.
