...
## Per-Tool Feasibility Assessments

Detailed feasibility assessments for all 20 built-in tools are in [feasibility/](feasibility/), grouped by complexity tier:

- [Trivial tools](feasibility/trivial-tools.md) — `read_frontmatter`, `get_outlinks`, `write_file`, `update_frontmatter`, `read_note`, `manage_tags`
- [Straightforward tools](feasibility/straightforward-tools.md) — `replace_in_file`, `execute_command`, `replace_in_note`, `write_note`, `list_vault`, `move_note`
- [Moderate tools](feasibility/moderate-tools.md) — `fetch_webpage`, `web_search`, `read_file`, `search_vault`, `read_docx`
- [Complex tools](feasibility/complex-tools.md) — `write_docx`, `extract_docx_comments`
