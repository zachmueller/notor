...
async function tests(ctx: TestContext) {
	...
	console.log("Test 1: Chat panel visibility");
	{
		...
		if (chatContainer) {
			...
			const shot = await ctx.screenshot("01-chat-panel-missing");
			ctx.fail("Chat panel visible", ".notor-chat-container not found; check if panel opened automatically", shot);
		}
	}
	...
	console.log("\nTest 2: Chat header and action buttons");
	{
		...
		if (header && newBtn && histBtn && settingsBtn) {
			ctx.pass("Header buttons present", "New conversation, History, and Settings buttons found");
		} else {
			ctx.fail("Header buttons present", `header=${!!header}, newBtn=${!!newBtn}, histBtn=${!!histBtn}, settingsBtn=${!!settingsBtn}`);
		}
	}
	...
	console.log("\nTest 3: Input area elements");
	{
		...
		if (textarea && sendBtn && modeToggle) {
			ctx.pass("Input area present", "Textarea, send button, and mode toggle found");
		} else {
			ctx.fail("Input area present", `textarea=${!!textarea}, sendBtn=${!!sendBtn}, modeToggle=${!!modeToggle}`);
		}
	}
	...
	console.log("\nTest 4: Plan/Act mode toggle — default state");
	{
		...
		if (modeToggle) {
			...
			if (text?.trim() === "Plan" && hasPlanClass) {
				ctx.pass("Mode toggle default is Plan", `text="${text?.trim()}", has notor-mode-plan class`);
			} else {
				ctx.fail("Mode toggle default is Plan", `text="${text?.trim()}", hasPlanClass=${hasPlanClass}`);
			}
		} else {
			ctx.fail("Mode toggle default is Plan", "Mode toggle element not found");
		}
	}
	...
	console.log("\nTest 5: Toggle Plan → Act");
	{
		...
		if (modeToggle) {
			...
			if (text?.trim() === "Act" && hasActClass) {
				const shot = await ctx.screenshot("05-act-mode");
				ctx.pass("Toggle Plan→Act", `text="${text?.trim()}", has notor-mode-act class`, shot);
			} else {
				ctx.fail("Toggle Plan→Act", `text="${text?.trim()}", hasActClass=${hasActClass}`);
			}
		} else {
			ctx.fail("Toggle Plan→Act", "Mode toggle element not found");
		}
	}
	...
	console.log("\nTest 6: Toggle Act → Plan");
	{
		...
		if (modeToggle) {
			...
			const hasPlanClass = await modeToggle.evaluate((el) => el.classList.contains("notor-mode-plan"));
			if (text?.trim() === "Plan" && hasPlanClass) {
				ctx.pass("Toggle Act→Plan", `text="${text?.trim()}", has notor-mode-plan class`);
			} else {
				ctx.fail("Toggle Act→Plan", `text="${text?.trim()}", hasPlanClass=${hasPlanClass}`);
			}
		} else {
			ctx.fail("Toggle Act→Plan", "Mode toggle element not found");
		}
	}
	...
	console.log("\nTest 7: Type in text input");
	{
		...
		if (textarea) {
			...
			if (value === "Hello from interaction test") {
				...
				ctx.fail("Type in textarea", `Expected "Hello from interaction test", got "${value}"`);
			}
		} else {
			ctx.fail("Type in textarea", "Textarea not found");
		}
	}
	...
	console.log("\nTest 8: Shift+Enter inserts newline");
	{
		...
		if (textarea) {
			...
			if (hasNewline) {
				...
				ctx.fail("Shift+Enter inserts newline", `No newline found in: ${JSON.stringify(value)}`);
			}
		} else {
			ctx.fail("Shift+Enter inserts newline", "Textarea not found");
		}
	}
	...
	console.log("\nTest 9: Send message → responding state (send→stop transition)");
	{
		...
		if (textarea) {
			...
			if (sendBtn) {
				...
				const shot = await ctx.screenshot("09-send-responding");

				if (stopVisible) {
					ctx.pass("Send triggers responding state", "Stop button visible after send (send→stop transition)");
				} else {
					const hasUserMsg = await page.$(".notor-message-user");
					if (hasUserMsg) {
						ctx.pass("Send triggers responding state", "User message rendered (provider may have errored out quickly)");
					} else {
						ctx.fail("Send triggers responding state", "Neither stop button visible nor user message found", shot);
					}
				}
			} else {
				ctx.fail("Send triggers responding state", "Send button not found");
			}
		} else {
			ctx.fail("Send triggers responding state", "Textarea not found");
		}
	}
	...
	console.log("\nTest 10: User message rendered in chat");
	{
		...
		if (userMsg) {
			...
			ctx.pass("User message rendered", `Found .notor-message-user with text: "${text?.trim().substring(0, 50)}"`);
		} else {
			ctx.fail("User message rendered", ".notor-message-user not found after send");
		}
	}
	...
	console.log("\nTest 11: Assistant response or error rendered");
	{
		...
		if (assistantMsg) {
			...
			ctx.pass("Assistant/error message rendered", `Found .notor-message-assistant: "${text?.trim().substring(0, 80)}"`);
		} else if (errorMsg) {
			const text = await errorMsg.textContent();
			ctx.pass("Assistant/error message rendered", `Found .notor-chat-error (expected with no LLM): "${text?.trim().substring(0, 80)}"`);
		} else {
			ctx.fail("Assistant/error message rendered", "No assistant message or error rendered after send");
		}
	}
	...
	console.log("\nTest 12: Input re-enabled after response completes");
	{
		...
		if (textarea) {
			...
			if (!isDisabled && stopHidden && sendVisible) {
				ctx.pass("Input re-enabled", "Textarea enabled, stop hidden, send visible");
			} else {
				ctx.fail("Input re-enabled", `textarea.disabled=${isDisabled}, stopHidden=${stopHidden}, sendVisible=${sendVisible}`);
			}
		} else {
			ctx.fail("Input re-enabled", "Textarea not found");
		}
	}
	...
	console.log("\nTest 13: New conversation button");
	{
		...
		if (newBtn) {
			...
			if (msgs.length === 0) {
				const shot = await ctx.screenshot("13-new-conversation");
				ctx.pass("New conversation clears messages", `Message list empty after new conversation`, shot);
			} else {
				ctx.fail("New conversation clears messages", `${msgs.length} messages still visible after new conversation`);
			}
		} else {
			ctx.fail("New conversation button", "New conversation button not found");
		}
	}
	...
	console.log("\nTest 14: Conversation history list toggle");
	{
		...
		if (histBtn) {
			...
			if (listVisible) {
				const shot = await ctx.screenshot("14-conversation-list");
				ctx.pass("Conversation list toggle (open)", "List visible, message list hidden", shot);
				...
				if (listHidden) {
					ctx.pass("Conversation list toggle (close)", "List hidden again after second click");
				} else {
					ctx.fail("Conversation list toggle (close)", "List still visible after second click");
				}
			} else {
				ctx.fail("Conversation list toggle (open)", `listVisible=${listVisible}, msgHidden=${msgHidden}`);
			}
		} else {
			ctx.fail("Conversation list toggle", "Conversation history button not found");
		}
	}
		...
		if (settingsBtn) {
			...
			if (popover) {
				...
				if (checkpointSection) {
					ctx.pass("Checkpoints section present", "Found .notor-checkpoints-section in popover");
				} else {
					ctx.fail("Checkpoints section present", "No .notor-checkpoints-section found");
				}
				...
				if (popoverGone) {
					ctx.pass("Settings popover closes", "Popover removed from DOM after second click");
				} else {
					ctx.fail("Settings popover closes", "Popover still present after second click");
				}
			} else {
				ctx.fail("Settings popover opens", ".notor-settings-popover not found after click");
			}
		} else {
			ctx.fail("Settings popover", "Settings button not found");
		}
	}
	...
	console.log("\nTest 16: Second message send in Plan mode");
	{
		...
		if (textarea) {
			...
			const shot = await ctx.screenshot("16-second-message");
			if (userMsgs.length >= 1) {
				ctx.pass("Second message sends correctly", `${userMsgs.length} user message(s) in chat`, shot);
			} else {
	...
	console.log("\nTest 17: Token footer present after message exchange");
	{
		...
		if (tokenFooter) {
			...
			if (!isHidden) {
				...
				ctx.pass("Token footer visible", `Footer text: "${text?.trim()}"`);
			} else {
				ctx.pass("Token footer present but hidden", "Footer element exists; hidden when no successful LLM response — expected with no provider configured");
			}
		} else {
			ctx.fail("Token footer present", ".notor-token-footer not found");
		}
	}
	...
	console.log("\nTest 18: Act mode toggle and write mode verification");
	{
		...
		if (modeToggle) {
			...
			const hasActClass = await modeToggle.evaluate((el) => el.classList.contains("notor-mode-act"));
			if (text?.trim() === "Act" && hasActClass) {
				const shot = await ctx.screenshot("18-act-mode");
				ctx.pass("Act mode toggled", `Mode is Act, has notor-mode-act class`, shot);

				await modeToggle.click();
				await page.waitForTimeout(300);
				ctx.pass("Returned to Plan mode", "Toggled back from Act to Plan");
			} else {
				ctx.fail("Act mode toggle", `text="${text?.trim()}", hasActClass=${hasActClass}`);
			}
		} else {
			ctx.fail("Act mode toggle", "Mode toggle not found");
		}
	}
	...
	console.log("\nTest 19: Multiple conversations in history list");
	{
		...
		if (newBtn) {
			...
			if (histBtn) {
				...
				const shot = await ctx.screenshot("19-conversation-history");
				if (items.length >= 1) {
					ctx.pass("Conversation history list", `${items.length} conversation(s) in history`, shot);
					...
					if (items.length > 0) {
						...
						if (listHidden) {
							ctx.pass("Switch conversation from history", "Clicked history item, list closed and chat visible");
						} else {
							ctx.fail("Switch conversation from history", "List still visible after clicking item");
						}
					}
				} else {
					ctx.fail("Conversation history list", "No conversation items found in history list", shot);
				}
			} else {
				ctx.fail("Multiple conversations in history", "History button not found");
			}
		} else {
			ctx.fail("Multiple conversations in history", "New conversation button not found");
		}
	}
}

runTest({ name: "interaction-test" }, tests).catch((err) => {
	console.error(err);
	process.exit(1);
});
