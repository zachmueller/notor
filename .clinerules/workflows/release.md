...
Push a new versioned release of the Notor Obsidian plugin. This workflow bumps the version, commits, tags, and pushes — triggering the GitHub Actions workflow that builds and creates a draft GitHub release.
...
```bash
git commit -m "[Cline] Version bump: {PREV_VERSION} → {NEW_VERSION}
...
```
https://github.com/zachmueller/notor/actions
```

Once the workflow completes, navigate to the Releases page:

```
https://github.com/zachmueller/notor/releases
```

Confirm that a **draft release** for `{NEW_VERSION}` exists with all three assets attached (`main.js`, `manifest.json`, `styles.css`).
