...
**Note:** This workflow should run BEFORE the `speckit-03-plan.md` workflow. Skipping clarification increases downstream rework risk.

## Workflow Steps

### Step 1: Load Feature Context

Determine the spec directory location and load specification:
...
```bash
# Verify spec.md exists in determined location
...
- Maximum 5 questions across entire session
...
**Prioritization Formula:** `Priority = Impact × Uncertainty`
...
Follow the commit standards defined in `.clinerules/git.md` to commit the updated specification file(s).
...
- Completed `speckit-01-specify.md` workflow
...
**Missing Spec File:** If no spec.md found, instruct user to run `speckit-01-specify.md` workflow first.

**No Meaningful Ambiguities:** If all categories are Clear or low-impact, report: "No critical ambiguities detected worth formal clarification" and suggest proceeding to planning.

**User Early Termination:** Respect user signals ("stop", "done", "proceed") and generate partial completion report.

**Quota Exceeded:** If 5 questions reached with high-impact categories unresolved, flag them under Deferred with rationale and recommend follow-up action.