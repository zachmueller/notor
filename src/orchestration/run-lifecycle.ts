...
/**
 * The launcher `chainToSuccessor` invokes for the successor run — the real
 * {@link launchOrchestration} in production, injectable in tests to observe the
 * gate decision without driving a full run.
 */
 ...
 *
 * **Blocked-handoff behavior (locked by run-lifecycle.test.ts, F6 §3.3):** a
 * blocked handoff (depth/budget exhausted) or a non-discoverable successor
 * surfaces a **Notice and stops the chain** — it does **not** launch a successor
 * and does **not** change the predecessor's status. The predecessor already
 * completed; only the further hop is skipped. (The earlier docstring claimed a
 * `FLOW_ERROR`/status-`error` stop; the code never mutated status — the docstring
 * was corrected to match the code, per F1's landed behavior.) The successor, when
 * launched, is a recovery **root**-able `origin: "chaining"` run (recovered as a
 * root once this predecessor is terminal — INT-005).
 *
 ...
 * the run registry (Fix 1) could own a detached chain.
 *
